...
public class PaymentService {
        ...
        if (!student) {
            ...
            if (feeBalance != 0) {
                if (feeBalance >= 100) {
                    ...
                    if (input.equals("y")) {
                        getReceipt(mailId, db.getName(mailId), db.getRollNum(mailId), db.getDepartment(mailId), db.getFeesPaid(mailId), db.getTotalFees(mailId), modeOfPayment);
                    }
                } else {
                    System.out.println("Your balance is " + feeBalance + ". It is less than 100...pay remaining balance with next payment.");
                }
            } else {
    ...
    public void viewBalance(String mailId) {
        ...
        if (db.getId(mailId)) {
            System.out.println("Name : " + db.getName(mailId) + "\nMail id:" + mailId + "\nRoll number:" + db.getRollNum(mailId) + "\ndepartment:" + db.getDepartment(mailId) + "\nFee balance:" + db.getFeesBalance(mailId));
        }


    }

    public void getReceipt(String mailId, String name, String rollNum, Department department, long feesPaid, long amount, PaymentConvenienceFees modeOfPayment) {
        ...
        System.out.println("------------------------------------------");
        System.out.println("\nAMOUNT PAID:" + amount + "\nMODE OF PAYMENT:" + modeOfPayment + "\nFEES BALANCE:" + db.getFeesBalance(mailId) + "\nFEES PAID TILL DATE:" + feesPaid);
